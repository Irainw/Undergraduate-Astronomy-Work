...
if __name__ == '__main__':
    ...
    axis = DateAxisItem(orientation='bottom',text='time')
    w.getPlotItem().setLabel(axis='bottom', text='Time')
    w.getPlotItem().setAxisItems({'bottom':axis})
    #axis.attachToPlotItem(w.getPlotItem())

    # plot some random data with timestamps in the last hour
    now = time.time()
    #timestamps = numpy.linspace(now - 36000, now, 10)
    timestamps= numpy.array([1670531964116123000,1670531954111613000,1670531944160236000,1670531934029133000,1670531923876357000,1665120009733952000,1665120007646358000,1665120005519514000,1665120003384741000,1665120001323244000])

    print(timestamps)
    w.plot(x=timestamps/10**9, y=numpy.random.rand(10), symbol='o')
