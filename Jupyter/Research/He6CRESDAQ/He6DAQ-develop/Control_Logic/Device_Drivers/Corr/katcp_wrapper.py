...
class FpgaClient(CallbackClient):
    ...
    def get_10gbe_core_details(self, dev_name):
        ...
        rv={}
        mymac=((port_dump[0o2]<<40) + (port_dump[0o3]<<32) + (port_dump[0o4]<<24) + (port_dump[0o5]<<16) + (port_dump[0o6]<<8) + port_dump[0o7])
        rv['mymac']=mymac
    ...
    def snapshot_get(self, dev_name, man_trig=False, man_valid=False, wait_period=10, offset=-1, circular_capture=False, get_extra_val=False, arm=True):
        """Grabs all brams from a single snap block on this FPGA device.\n
            \tdev_name: string, name of the snap block.\n
            \tman_trig: boolean, Trigger the snap block manually.\n
