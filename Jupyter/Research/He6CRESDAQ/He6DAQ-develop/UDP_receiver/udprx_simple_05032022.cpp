...
int main(int argc, char *argv[4]) {
  ...
  char *buffer = new char [BUFFERSIZE*tot_packets];
  // cout << "Got here 2." ;
  ...
  // cout << "Got here." << buffer + RxPackets*BUFFERSIZE << ",";
  while(RxPackets < tot_packets){

      //pull individual packet off the network interface
      int ReadSize = Receive.receive(buffer + RxPackets*BUFFERSIZE, BUFFERSIZE);
    ...
    // Copy the intermediate buffer to the vector: 
    std::copy (buffer, buffer+RxPackets*BUFFERSIZE, sdb_iter);

    delete []buffer;     // This is necesary when using new to prevent memory leaks. 
