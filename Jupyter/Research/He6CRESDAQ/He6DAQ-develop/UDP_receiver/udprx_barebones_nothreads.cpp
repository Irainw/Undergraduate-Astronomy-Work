...
struct {
  int UDPPort{4003};
  int DataSize{5000};
  int SocketBufferSize{700000000};
} Settings;
...
int main(int argc, char *argv[3]) {
  ...
  std::chrono::milliseconds downtime(2000); // Currently 1s. How long to rest between threads. 
  ...
  while(outputs < intervals) {
    ...
    //fileWriter(outFileStringB, sdb_datablock, RxPackets);
